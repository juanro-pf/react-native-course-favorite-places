...
export default function Map({ navigation, route }) {
    ...
    const [selectedLocation, setSetselectedLocation] = useState(initialLocation);
    ...
    function selectLocationHandler(event) {
        ...
        setSetselectedLocation({ lat, lng });
    };

    const savePickedLocationHandler = useCallback(() => {
        if (!selectedLocation) {
            Alert.alert("No location picked", "Tap a location on the map first.");
            return;
        }
